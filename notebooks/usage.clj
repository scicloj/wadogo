...
(clerk/hide-result
 (comment
   ...
   (clerk/build-static-app! {:paths ["notebooks/usage.clj"] :out-path "docs"})))

(+ 1 2 3 5) 

^{:nextjournal.clerk/visibility #{:fold}}
(clerk/hide-result (defn cont->cont
                     [scale]
...
(clerk/vl (cont->cont (s/scale :linear {:ticks [0.2 0.5 0.9]})))
...
(clerk/vl (cont->cont (s/scale :log {:base 2
                                     :domain [1 16]})))


(clerk/vl (cont->cont (s/scale :symlog {:domain [-5 5]})))

(clerk/vl (cont->cont (s/scale :pow)))

(clerk/vl (cont->cont (s/scale :pow {:exponent 2.0})))


(clerk/vl (cont->cont (s/scale :interpolated {:domain [1 2 4 6 10]
                                              :range [-7 0 2 4 10]})))
